...
public class FragmentOrderDishOrderDishAddonsList extends FragmentOrderDishAddonsList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityOrderDishDetails.ID);
			return new WrapperRepOrderDishAddons_OrderDishId(RepositoryManager.getInstance().getIOrderDishAddonsRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityOrderDishDetails.ID);
			return new WrapperRepOrderDishAddons_OrderDishId(RepositoryManager.getInstance().getIOrderDishAddonsRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.orderdishaddons.ActivityOrderDishAddonsList.class)
					...
					.putExtra("REPOSITORY_ORDERDISHADDONS", FragmentOrderDishOrderDishAddonsList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.orderdishaddons.ActivityOrderDishAddonsEdit.class)
					.putExtra("REPOSITORY_ORDERDISHADDONS", FragmentOrderDishOrderDishAddonsList.Factory.class)
					.putExtra(ActivityOrderDishDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityOrderDishDetails.ID))
					.putExtra(BindableEditFragment.READ_ONLY_FIELDS, new String[] {"OrderDishId"} )
					, com.delivery2go.base.orderdishaddons.FragmentOrderDishAddonsEdit.ORDERDISHADDONS_EDITED);
				return true;
		}
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		int[] ids =(int[])getActivity().getIntent().getSerializableExtra(ActivityOrderDishDetails.ID);

		return new ViewModelOrderDishAddonsList(this, new WrapperRepOrderDishAddons_OrderDishId(RepositoryManager.getInstance().getIOrderDishAddonsRepository(), ids[0]));
	}
