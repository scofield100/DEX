...
public class ActivityUserDetails extends Activity implements ActionBar.TabListener {
	...
	public class SectionsPagerAdapter extends FragmentPagerAdapter {
		...
		public SectionsPagerAdapter(FragmentManager fm) {
			...
			addPages(new Class[]{
					FragmentUserDetails.class
					,FragmentUserPermissionList.class
					,FragmentUserEntitiesEntityList.class
		...
		void addPages(Class<?>[]fragments, String[]labels){
			...
			for (int i = 0; i < fragments.length; i++) {
				if(i==0)
                    ...
                    if(Access.hasAccess(Rolls.Administrator, Access.READ))
                        pages.add(new FragmentPage(getApplicationContext(), fragments[i], labels[i], null, null));
                }
				else if(Access.hasAccess(fragments[i], Access.READ))
					pages.add(new FragmentPage(getApplicationContext(), fragments[i], labels[i], null, null));
			}
		}
