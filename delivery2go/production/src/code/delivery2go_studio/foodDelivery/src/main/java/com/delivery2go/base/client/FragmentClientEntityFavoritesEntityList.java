...
public class FragmentClientEntityFavoritesEntityList extends FragmentEntityList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			...
			return RepositoryManager.getInstance().getClientEntityFavorites_EntityRepository(ids[0], false);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityClientDetails.ID);
			return RepositoryManager.getInstance().getClientEntityFavorites_EntityRepository(ids[0], true);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.entity.ActivityEntityList.class)
					...
					.putExtra("REPOSITORY_ENTITY", FragmentClientEntityFavoritesEntityList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.entity.ActivityEntityEdit.class)
					.putExtra("REPOSITORY_ENTITY", FragmentClientEntityFavoritesEntityList.Factory.class)
					.putExtra(ActivityClientDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityClientDetails.ID)), com.delivery2go.base.entity.FragmentEntityEdit.ENTITY_EDITED);
				return true;
		}
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		...
		return new ViewModelEntityList(this, RepositoryManager.getInstance().getClientEntityFavorites_EntityRepository(ids[0], false));
	}
