...
public class FragmentStateEntityList extends FragmentEntityList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			...
			return new WrapperRepEntity_StateId(RepositoryManager.getInstance().getIEntityRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityStateDetails.ID);
			return new WrapperRepEntity_StateId(RepositoryManager.getInstance().getIEntityRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.entity.ActivityEntityList.class)
					...
					.putExtra("REPOSITORY_ENTITY", FragmentStateEntityList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.entity.ActivityEntityEdit.class)
					.putExtra("REPOSITORY_ENTITY", FragmentStateEntityList.Factory.class)
					.putExtra(ActivityStateDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityStateDetails.ID))
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		...
		return new ViewModelEntityList(this, new WrapperRepEntity_StateId(RepositoryManager.getInstance().getIEntityRepository(), ids[0]));
	}
