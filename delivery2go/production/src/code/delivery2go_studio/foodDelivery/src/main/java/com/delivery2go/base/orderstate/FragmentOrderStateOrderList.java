...
public class FragmentOrderStateOrderList extends FragmentOrderList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityOrderStateDetails.ID);
			return new WrapperRepOrder_OrderStateId(RepositoryManager.getInstance().getIOrderRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityOrderStateDetails.ID);
			return new WrapperRepOrder_OrderStateId(RepositoryManager.getInstance().getIOrderRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.order.ActivityOrderList.class)
					...
					.putExtra("REPOSITORY_ORDER", FragmentOrderStateOrderList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.order.ActivityOrderEdit.class)
					.putExtra("REPOSITORY_ORDER", FragmentOrderStateOrderList.Factory.class)
					.putExtra(ActivityOrderStateDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityOrderStateDetails.ID))
					.putExtra(BindableEditFragment.READ_ONLY_FIELDS, new String[] {"OrderStateId"} )
					, com.delivery2go.base.order.FragmentOrderEdit.ORDER_EDITED);
				return true;
		}
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		int[] ids =(int[])getActivity().getIntent().getSerializableExtra(ActivityOrderStateDetails.ID);

		return new ViewModelOrderList(this, new WrapperRepOrder_OrderStateId(RepositoryManager.getInstance().getIOrderRepository(), ids[0]), false);
	}
