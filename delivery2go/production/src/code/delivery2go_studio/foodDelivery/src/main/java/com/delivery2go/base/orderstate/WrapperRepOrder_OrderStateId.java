...
public class WrapperRepOrder_OrderStateId extends WrapperRepository<Order> implements IOrderRepository , IAttachRepository<Order>{

	int id;
	String filter;
	public WrapperRepOrder_OrderStateId(IOrderRepository repository, int id){
		super(repository);
		this.id = id;
		filter = String.format("OrderStateId = %d", id);
	}

	public WrapperRepOrder_OrderStateId(IOrderRepository repository, int id, int op){
		this(repository,id);
		filter = String.format("OrderStateId %s %d", op == FilterOp.EQUALS ? "=": "!=", id);	}
