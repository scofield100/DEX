...
public class FragmentDishSizeDishSizePriceList extends FragmentDishSizePriceList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityDishSizeDetails.ID);
			return new WrapperRepDishSizePrice_SizeId(RepositoryManager.getInstance().getIDishSizePriceRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityDishSizeDetails.ID);
			return new WrapperRepDishSizePrice_SizeId(RepositoryManager.getInstance().getIDishSizePriceRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.dishsizeprice.ActivityDishSizePriceList.class)
					...
					.putExtra("REPOSITORY_DISHSIZEPRICE", FragmentDishSizeDishSizePriceList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.dishsizeprice.ActivityDishSizePriceEdit.class)
					.putExtra("REPOSITORY_DISHSIZEPRICE", FragmentDishSizeDishSizePriceList.Factory.class)
					.putExtra(ActivityDishSizeDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityDishSizeDetails.ID))
					.putExtra(BindableEditFragment.READ_ONLY_FIELDS, new String[] {"SizeId"} )
					, com.delivery2go.base.dishsizeprice.FragmentDishSizePriceEdit.DISHSIZEPRICE_EDITED);
				return true;
		}
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		int[] ids =(int[])getActivity().getIntent().getSerializableExtra(ActivityDishSizeDetails.ID);

		return new ViewModelDishSizePriceList(this, new WrapperRepDishSizePrice_SizeId(RepositoryManager.getInstance().getIDishSizePriceRepository(), ids[0]));
	}
