...
public class FragmentOrderOrderDishList extends FragmentOrderDishList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityOrderDetails.ID);
			return new WrapperRepOrderDish_OrderId(RepositoryManager.getInstance().getIOrderDishRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityOrderDetails.ID);
			return new WrapperRepOrderDish_OrderId(RepositoryManager.getInstance().getIOrderDishRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
	@Override
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
		inflater.inflate(R.menu.menu_dishorder_list, menu);
...
//	}
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.orderdish.ActivityOrderDishList.class)
					...
					.putExtra("REPOSITORY_ORDERDISH", FragmentOrderOrderDishList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.orderdish.ActivityOrderDishEdit.class)
					.putExtra("REPOSITORY_ORDERDISH", FragmentOrderOrderDishList.Factory.class)
					.putExtra(ActivityOrderDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityOrderDetails.ID))
					.putExtra(BindableEditFragment.READ_ONLY_FIELDS, new String[] {"OrderId"} )
					, com.delivery2go.base.orderdish.FragmentOrderDishEdit.ORDERDISH_EDITED);
				return true;
		}
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		int[] ids =(int[])getActivity().getIntent().getSerializableExtra(ActivityOrderDetails.ID);

		return new ViewModelOrderDishList(this, new WrapperRepOrderDish_OrderId(RepositoryManager.getInstance().getIOrderDishRepository(), ids[0]));
	}
