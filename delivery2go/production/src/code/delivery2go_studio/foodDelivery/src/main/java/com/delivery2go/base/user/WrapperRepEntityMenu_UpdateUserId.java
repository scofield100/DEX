...
public class WrapperRepEntityMenu_UpdateUserId extends WrapperRepository<EntityMenu> implements IEntityMenuRepository , IAttachRepository<EntityMenu>{

	int id;
	String filter;
	public WrapperRepEntityMenu_UpdateUserId(IEntityMenuRepository repository, int id){
		super(repository);
		this.id = id;
		filter = String.format("UpdateUserId = %d", id);
	}

	public WrapperRepEntityMenu_UpdateUserId(IEntityMenuRepository repository, int id, int op){
		this(repository,id);
