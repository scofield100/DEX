...
public class FragmentUserDishSizePriceList extends FragmentDishSizePriceList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			...
			return new WrapperRepDishSizePrice_UpdateUserId(RepositoryManager.getInstance().getIDishSizePriceRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityUserDetails.ID);
			return new WrapperRepDishSizePrice_UpdateUserId(RepositoryManager.getInstance().getIDishSizePriceRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.dishsizeprice.ActivityDishSizePriceList.class)
					...
					.putExtra("REPOSITORY_DISHSIZEPRICE", FragmentUserDishSizePriceList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.dishsizeprice.ActivityDishSizePriceEdit.class)
					.putExtra("REPOSITORY_DISHSIZEPRICE", FragmentUserDishSizePriceList.Factory.class)
					.putExtra(ActivityUserDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityUserDetails.ID))
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		...
		return new ViewModelDishSizePriceList(this, new WrapperRepDishSizePrice_UpdateUserId(RepositoryManager.getInstance().getIDishSizePriceRepository(), ids[0]));
	}
