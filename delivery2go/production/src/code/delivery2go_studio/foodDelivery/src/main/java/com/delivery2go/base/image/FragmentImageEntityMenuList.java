...
public class FragmentImageEntityMenuList extends FragmentEntityMenuList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			...
			return new WrapperRepEntityMenu_ImageId(RepositoryManager.getInstance().getIEntityMenuRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityImageDetails.ID);
			return new WrapperRepEntityMenu_ImageId(RepositoryManager.getInstance().getIEntityMenuRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.entitymenu.ActivityEntityMenuList.class)
					...
					.putExtra("REPOSITORY_ENTITYMENU", FragmentImageEntityMenuList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.entitymenu.ActivityEntityMenuEdit.class)
					.putExtra("REPOSITORY_ENTITYMENU", FragmentImageEntityMenuList.Factory.class)
					.putExtra(ActivityImageDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityImageDetails.ID))
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		...
		return new ViewModelEntityMenuList(this, new WrapperRepEntityMenu_ImageId(RepositoryManager.getInstance().getIEntityMenuRepository(), ids[0]));
	}
