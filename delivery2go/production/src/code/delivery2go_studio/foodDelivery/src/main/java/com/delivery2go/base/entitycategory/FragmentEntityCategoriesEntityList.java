...
public class FragmentEntityCategoriesEntityList extends FragmentEntityList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityEntityCategoryDetails.ID);
			return RepositoryManager.getInstance().getEntityCategories_EntityRepository(ids[0], false);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityEntityCategoryDetails.ID);
			return RepositoryManager.getInstance().getEntityCategories_EntityRepository(ids[0], true);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.entity.ActivityEntityList.class)
					...
					.putExtra("REPOSITORY_ENTITY", FragmentEntityCategoriesEntityList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.entity.ActivityEntityEdit.class)
					.putExtra("REPOSITORY_ENTITY", FragmentEntityCategoriesEntityList.Factory.class)
					.putExtra(ActivityEntityCategoryDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityEntityCategoryDetails.ID)), com.delivery2go.base.entity.FragmentEntityEdit.ENTITY_EDITED);
				return true;
		}
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		int[] ids =(int[])getActivity().getIntent().getSerializableExtra(ActivityEntityCategoryDetails.ID);

		return new ViewModelEntityList(this, RepositoryManager.getInstance().getEntityCategories_EntityRepository(ids[0], false));
	}
