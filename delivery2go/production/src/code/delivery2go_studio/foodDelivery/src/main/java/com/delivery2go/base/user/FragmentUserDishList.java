...
public class FragmentUserDishList extends FragmentDishList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			...
			return new WrapperRepDish_UpdateUserId(RepositoryManager.getInstance().getIDishRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityUserDetails.ID);
			return new WrapperRepDish_UpdateUserId(RepositoryManager.getInstance().getIDishRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.dish.ActivityDishList.class)
					...
					.putExtra("REPOSITORY_DISH", FragmentUserDishList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.dish.ActivityDishEdit.class)
					.putExtra("REPOSITORY_DISH", FragmentUserDishList.Factory.class)
					.putExtra(ActivityUserDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityUserDetails.ID))
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		...
		return new ViewModelDishList(this, new WrapperRepDish_UpdateUserId(RepositoryManager.getInstance().getIDishRepository(), ids[0]));
	}
