...
public class FragmentEntityAddonsList extends FragmentAddonsList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			...
			return new WrapperRepAddons_EntityId(RepositoryManager.getInstance().getIAddonsRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityEntityDetails.ID);
			return new WrapperRepAddons_EntityId(RepositoryManager.getInstance().getIAddonsRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.addons.ActivityAddonsList.class)
					...
					.putExtra("REPOSITORY_ADDONS", FragmentEntityAddonsList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.addons.ActivityAddonsEdit.class)
					.putExtra("REPOSITORY_ADDONS", FragmentEntityAddonsList.Factory.class)
					.putExtra(ActivityEntityDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityEntityDetails.ID))
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		...
		return new ViewModelAddonsList(this, new WrapperRepAddons_EntityId(RepositoryManager.getInstance().getIAddonsRepository(), ids[0]));
	}
