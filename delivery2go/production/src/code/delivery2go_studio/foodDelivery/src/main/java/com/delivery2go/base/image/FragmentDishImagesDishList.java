...
public class FragmentDishImagesDishList extends FragmentDishList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			...
			return RepositoryManager.getInstance().getDishImages_DishRepository(ids[0], false);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityImageDetails.ID);
			return RepositoryManager.getInstance().getDishImages_DishRepository(ids[0], true);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.dish.ActivityDishList.class)
					...
					.putExtra("REPOSITORY_DISH", FragmentDishImagesDishList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.dish.ActivityDishEdit.class)
					.putExtra("REPOSITORY_DISH", FragmentDishImagesDishList.Factory.class)
					.putExtra(ActivityImageDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityImageDetails.ID)), com.delivery2go.base.dish.FragmentDishEdit.DISH_EDITED);
				return true;
		}
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		...
		return new ViewModelDishList(this, RepositoryManager.getInstance().getDishImages_DishRepository(ids[0], false));
	}
