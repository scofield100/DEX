...
public class WrapperRepAddons_EntityId extends WrapperRepository<Addons> implements IAddonsRepository , IAttachRepository<Addons>{

	int id;
	String filter;
	public WrapperRepAddons_EntityId(IAddonsRepository repository, int id){
		super(repository);
		this.id = id;
		filter = String.format("EntityId = %d", id);
	}

	public WrapperRepAddons_EntityId(IAddonsRepository repository, int id, int op){
		this(repository,id);
