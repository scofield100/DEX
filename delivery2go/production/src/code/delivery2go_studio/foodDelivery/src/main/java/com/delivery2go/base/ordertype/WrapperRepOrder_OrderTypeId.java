...
public class WrapperRepOrder_OrderTypeId extends WrapperRepository<Order> implements IOrderRepository , IAttachRepository<Order>{

	int id;
	String filter;
	public WrapperRepOrder_OrderTypeId(IOrderRepository repository, int id){
		super(repository);
		this.id = id;
		filter = String.format("OrderTypeId = %d", id);
	}

	public WrapperRepOrder_OrderTypeId(IOrderRepository repository, int id, int op){
		this(repository,id);
		filter = String.format("OrderTypeId %s %d", op == FilterOp.EQUALS ? "=": "!=", id);	}
