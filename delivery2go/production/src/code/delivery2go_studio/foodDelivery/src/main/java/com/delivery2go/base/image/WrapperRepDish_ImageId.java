...
public class WrapperRepDish_ImageId extends WrapperRepository<Dish> implements IDishRepository , IAttachRepository<Dish>{

	int id;
	String filter;
	public WrapperRepDish_ImageId(IDishRepository repository, int id){
		super(repository);
		this.id = id;
		filter = String.format("ImageId = %d", id);
	}

	public WrapperRepDish_ImageId(IDishRepository repository, int id, int op){
		this(repository,id);
