package com.delivery2go.base.data;

import com.enterlib.data.EntityMap;
import com.enterlib.data.IEntityContext;
import com.enterlib.data.IEntityMapInitializer;
import com.enterlib.data.generics.IFactory;
import com.delivery2go.base.models.DishCategory;

public class DishCategoryFactory implements IEntityMapInitializer<DishCategory> ,IFactory<DishCategory>{

	protected IEntityContext context;
	public DishCategoryFactory(IEntityContext context){
		this.context = context;
	}
	@Override
	public void init(EntityMap<DishCategory> map){
		map.setFactory(this);
	}

	@Override
	public DishCategory getInstance(){
		//TODO: return your custum model class here 

		return new DishCategory(context);
	}

}

