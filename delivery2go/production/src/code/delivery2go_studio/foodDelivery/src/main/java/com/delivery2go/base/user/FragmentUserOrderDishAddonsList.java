...
public class FragmentUserOrderDishAddonsList extends FragmentOrderDishAddonsList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			...
			return new WrapperRepOrderDishAddons_UpdateUserId(RepositoryManager.getInstance().getIOrderDishAddonsRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityUserDetails.ID);
			return new WrapperRepOrderDishAddons_UpdateUserId(RepositoryManager.getInstance().getIOrderDishAddonsRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.orderdishaddons.ActivityOrderDishAddonsList.class)
					...
					.putExtra("REPOSITORY_ORDERDISHADDONS", FragmentUserOrderDishAddonsList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.orderdishaddons.ActivityOrderDishAddonsEdit.class)
					.putExtra("REPOSITORY_ORDERDISHADDONS", FragmentUserOrderDishAddonsList.Factory.class)
					.putExtra(ActivityUserDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityUserDetails.ID))
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		...
		return new ViewModelOrderDishAddonsList(this, new WrapperRepOrderDishAddons_UpdateUserId(RepositoryManager.getInstance().getIOrderDishAddonsRepository(), ids[0]));
	}
