...
public class FragmentUserOrderDishList extends FragmentOrderDishList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			...
			return new WrapperRepOrderDish_UpdateUserId(RepositoryManager.getInstance().getIOrderDishRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityUserDetails.ID);
			return new WrapperRepOrderDish_UpdateUserId(RepositoryManager.getInstance().getIOrderDishRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.orderdish.ActivityOrderDishList.class)
					...
					.putExtra("REPOSITORY_ORDERDISH", FragmentUserOrderDishList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.orderdish.ActivityOrderDishEdit.class)
					.putExtra("REPOSITORY_ORDERDISH", FragmentUserOrderDishList.Factory.class)
					.putExtra(ActivityUserDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityUserDetails.ID))
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		...
		return new ViewModelOrderDishList(this, new WrapperRepOrderDish_UpdateUserId(RepositoryManager.getInstance().getIOrderDishRepository(), ids[0]));
	}
