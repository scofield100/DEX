...
public class FragmentUserOrderList extends FragmentOrderList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			...
			return new WrapperRepOrder_UpdateUserId(RepositoryManager.getInstance().getIOrderRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityUserDetails.ID);
			return new WrapperRepOrder_UpdateUserId(RepositoryManager.getInstance().getIOrderRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.order.ActivityOrderList.class)
					...
					.putExtra("REPOSITORY_ORDER", FragmentUserOrderList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.order.ActivityOrderEdit.class)
					.putExtra("REPOSITORY_ORDER", FragmentUserOrderList.Factory.class)
					.putExtra(ActivityUserDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityUserDetails.ID))
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		...
		return new ViewModelOrderList(this, new WrapperRepOrder_UpdateUserId(RepositoryManager.getInstance().getIOrderRepository(), ids[0]), false);
	}
