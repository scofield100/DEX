...
public class WrapperRepOrder_UpdateUserId extends WrapperRepository<Order> implements IOrderRepository , IAttachRepository<Order>{

	int id;
	String filter;
	public WrapperRepOrder_UpdateUserId(IOrderRepository repository, int id){
		super(repository);
		this.id = id;
		filter = String.format("UpdateUserId = %d", id);
	}

	public WrapperRepOrder_UpdateUserId(IOrderRepository repository, int id, int op){
		this(repository,id);
