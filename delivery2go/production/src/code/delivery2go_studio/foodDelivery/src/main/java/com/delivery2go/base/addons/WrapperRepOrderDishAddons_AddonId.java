...
public class WrapperRepOrderDishAddons_AddonId extends WrapperRepository<OrderDishAddons> implements IOrderDishAddonsRepository , IAttachRepository<OrderDishAddons>{

	int id;
	String filter;
	public WrapperRepOrderDishAddons_AddonId(IOrderDishAddonsRepository repository, int id){
		super(repository);
		this.id = id;
		filter = String.format("AddonId = %d", id);
	}

	public WrapperRepOrderDishAddons_AddonId(IOrderDishAddonsRepository repository, int id, int op){
		this(repository,id);
		filter = String.format("AddonId %s %d", op == FilterOp.EQUALS ? "=": "!=", id);	}
