...
public class FragmentRollPermissionList extends FragmentPermissionList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityRollDetails.ID);
			return new WrapperRepPermission_RollId(RepositoryManager.getInstance().getIPermissionRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityRollDetails.ID);
			return new WrapperRepPermission_RollId(RepositoryManager.getInstance().getIPermissionRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.permission.ActivityPermissionList.class)
					...
					.putExtra("REPOSITORY_PERMISSION", FragmentRollPermissionList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.permission.ActivityPermissionEdit.class)
					.putExtra("REPOSITORY_PERMISSION", FragmentRollPermissionList.Factory.class)
					.putExtra(ActivityRollDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityRollDetails.ID))
					.putExtra(BindableEditFragment.READ_ONLY_FIELDS, new String[] {"RollId"} )
					, com.delivery2go.base.permission.FragmentPermissionEdit.PERMISSION_EDITED);
				return true;
		}
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		int[] ids =(int[])getActivity().getIntent().getSerializableExtra(ActivityRollDetails.ID);

		return new ViewModelPermissionList(this, new WrapperRepPermission_RollId(RepositoryManager.getInstance().getIPermissionRepository(), ids[0]));
	}
