...
public class FragmentDishSizePriceOrderDishList extends FragmentOrderDishList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityDishSizePriceDetails.ID);
			return new WrapperRepOrderDish_SizeId(RepositoryManager.getInstance().getIOrderDishRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityDishSizePriceDetails.ID);
			return new WrapperRepOrderDish_SizeId(RepositoryManager.getInstance().getIOrderDishRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.orderdish.ActivityOrderDishList.class)
					...
					.putExtra("REPOSITORY_ORDERDISH", FragmentDishSizePriceOrderDishList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.orderdish.ActivityOrderDishEdit.class)
					.putExtra("REPOSITORY_ORDERDISH", FragmentDishSizePriceOrderDishList.Factory.class)
					.putExtra(ActivityDishSizePriceDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityDishSizePriceDetails.ID))
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		int[] ids =(int[])getActivity().getIntent().getSerializableExtra(ActivityDishSizePriceDetails.ID);

		return new ViewModelOrderDishList(this, new WrapperRepOrderDish_SizeId(RepositoryManager.getInstance().getIOrderDishRepository(), ids[0]));
	}
