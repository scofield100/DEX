...
public class FragmentDishImagesImageList extends FragmentImageList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			...
			return RepositoryManager.getInstance().getDishImages_ImageRepository(ids[0], false);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityDishDetails.ID);
			return RepositoryManager.getInstance().getDishImages_ImageRepository(ids[0], true);
		}
	}
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		...
		return new ViewModelImageList(this, RepositoryManager.getInstance().getDishImages_ImageRepository(ids[0], false));
	}
