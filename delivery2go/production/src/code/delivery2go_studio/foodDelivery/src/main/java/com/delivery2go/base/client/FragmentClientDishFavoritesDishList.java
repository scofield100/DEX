...
public class FragmentClientDishFavoritesDishList extends FragmentDishList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			...
			return RepositoryManager.getInstance().getClientDishFavorites_DishRepository(ids[0], false);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityClientDetails.ID);
			return RepositoryManager.getInstance().getClientDishFavorites_DishRepository(ids[0], true);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.dish.ActivityDishList.class)
					...
					.putExtra("REPOSITORY_DISH", FragmentClientDishFavoritesDishList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.dish.ActivityDishEdit.class)
					.putExtra("REPOSITORY_DISH", FragmentClientDishFavoritesDishList.Factory.class)
					.putExtra(ActivityClientDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityClientDetails.ID)), com.delivery2go.base.dish.FragmentDishEdit.DISH_EDITED);
				return true;
		}
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		...
		return new ViewModelDishList(this, RepositoryManager.getInstance().getClientDishFavorites_DishRepository(ids[0], false));
	}
