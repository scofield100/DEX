...
public class WrapperRepOrder_ClientId extends WrapperRepository<Order> implements IOrderRepository , IAttachRepository<Order>{

	int id;
	String filter;
	public WrapperRepOrder_ClientId(IOrderRepository repository, int id){
		super(repository);
		this.id = id;
		filter = String.format("ClientId = %d", id);
	}

	public WrapperRepOrder_ClientId(IOrderRepository repository, int id, int op){
		this(repository,id);
