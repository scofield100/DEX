...
public class FragmentCityEntityList extends FragmentEntityList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityCityDetails.ID);
			return new WrapperRepEntity_CityId(RepositoryManager.getInstance().getIEntityRepository(), ids[0]);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityCityDetails.ID);
			return new WrapperRepEntity_CityId(RepositoryManager.getInstance().getIEntityRepository(), ids[0], FilterOp.NOT_EQUALS);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.entity.ActivityEntityList.class)
					...
					.putExtra("REPOSITORY_ENTITY", FragmentCityEntityList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.entity.ActivityEntityEdit.class)
					.putExtra("REPOSITORY_ENTITY", FragmentCityEntityList.Factory.class)
					.putExtra(ActivityCityDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityCityDetails.ID))
					.putExtra(BindableEditFragment.READ_ONLY_FIELDS, new String[] {"CityId"} )
					, com.delivery2go.base.entity.FragmentEntityEdit.ENTITY_EDITED);
				return true;
		}
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		int[] ids =(int[])getActivity().getIntent().getSerializableExtra(ActivityCityDetails.ID);

		return new ViewModelEntityList(this, new WrapperRepEntity_CityId(RepositoryManager.getInstance().getIEntityRepository(), ids[0]));
	}
