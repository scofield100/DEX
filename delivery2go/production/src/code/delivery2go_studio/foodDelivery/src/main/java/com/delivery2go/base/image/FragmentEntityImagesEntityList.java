...
public class FragmentEntityImagesEntityList extends FragmentEntityList{
	public static class Factory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			...
			return RepositoryManager.getInstance().getEntityImages_EntityRepository(ids[0], false);
		}
	}
	public static class AttachFactory implements IRepositoryFactory{
		public Object getInstance(Activity activity, Fragment fragment) {
			int[] ids =(int[])activity.getIntent().getSerializableExtra(ActivityImageDetails.ID);
			return RepositoryManager.getInstance().getEntityImages_EntityRepository(ids[0], true);
		}
	}
	@Override
		...
		switch (item.getItemId()){
			...
			case R.id.attach:
				startActivityForResult(new  Intent(getActivity().getApplicationContext(), com.delivery2go.base.entity.ActivityEntityList.class)
					...
					.putExtra("REPOSITORY_ENTITY", FragmentEntityImagesEntityList.AttachFactory.class)
			...
			case R.id.create:
				startActivityForResult(new Intent(getActivity().getApplicationContext(), com.delivery2go.base.entity.ActivityEntityEdit.class)
					.putExtra("REPOSITORY_ENTITY", FragmentEntityImagesEntityList.Factory.class)
					.putExtra(ActivityImageDetails.ID, (int[])getActivity().getIntent().getSerializableExtra(ActivityImageDetails.ID)), com.delivery2go.base.entity.FragmentEntityEdit.ENTITY_EDITED);
				return true;
		}
	...
	protected DataViewModel createViewModel(Bundle savedInstanceState) {
		...
		return new ViewModelEntityList(this, RepositoryManager.getInstance().getEntityImages_EntityRepository(ids[0], false));
	}
