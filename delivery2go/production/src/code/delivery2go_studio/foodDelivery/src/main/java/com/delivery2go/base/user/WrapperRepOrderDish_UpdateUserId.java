...
public class WrapperRepOrderDish_UpdateUserId extends WrapperRepository<OrderDish> implements IOrderDishRepository , IAttachRepository<OrderDish>{

	int id;
	String filter;
	public WrapperRepOrderDish_UpdateUserId(IOrderDishRepository repository, int id){
		super(repository);
		this.id = id;
		filter = String.format("UpdateUserId = %d", id);
	}

	public WrapperRepOrderDish_UpdateUserId(IOrderDishRepository repository, int id, int op){
		this(repository,id);
